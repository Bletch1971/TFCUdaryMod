...
import udary.tfcudarymod.core.enums.EnumTuckerBagVersion;
import udary.tfcudarymod.handlers.FuelHandler;
import udary.tfcudarymod.items.ItemEmpty;
import udary.tfcudarymod.items.devices.ItemPotteryBattery;
import udary.tfcudarymod.items.fluids.ItemBlueSteelBucketFluid;
import udary.tfcudarymod.items.fluids.ItemCeramicJugMilk;
import udary.tfcudarymod.items.fluids.ItemGlassBottleFluid;
import udary.tfcudarymod.items.fluids.ItemGlassBottleMilk;
import udary.tfcudarymod.items.fluids.ItemGlassBottleWater;
import udary.tfcudarymod.items.fluids.ItemRedSteelBucketFluid;
import udary.tfcudarymod.items.fluids.ItemWoodenBucketFluid;
import udary.tfcudarymod.items.materials.ItemBuckle;
import udary.tfcudarymod.items.materials.ItemWire;
import udary.tfcudarymod.items.ores.ItemOre;
import udary.tfcudarymod.items.ores.ItemOreFlake;
import udary.tfcudarymod.items.pottery.ItemPotteryMold;
import udary.tfcudarymod.items.powders.ItemPowder;
import udary.tfcudarymod.items.terrafirmacraft.ItemTFCLog;
import udary.tfcudarymod.items.tools.ItemCeramicBucket;
import udary.tfcudarymod.items.tools.ItemRod;
import udary.tfcudarymod.items.tools.ItemTuckerBag;
...
public class ModItems 
{
	...
	private static void registerTFCItems()
	{
		if (ModOptions.enableLogPileMod)
		{
        	try 
        	{
        		String key = GameData.getItemRegistry().getNameForObject(TFCItems.logs);
    			String name = TFCItems.logs.getUnlocalizedName().replace("item.", "");
    			
    			TfcLog = new ItemTFCLog().setUnlocalizedName(name);
        		
				GameRegistry.addSubstitutionAlias(key, GameRegistry.Type.ITEM, TfcLog);
			} 
        	catch (Exception e) 
        	{
        		System.out.println("** ERROR ** [" + ModDetails.ModName + "] addSubstitutionAlias failed (TfcLog).");

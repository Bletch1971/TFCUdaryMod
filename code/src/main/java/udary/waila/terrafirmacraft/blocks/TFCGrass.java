...
public class TFCGrass implements IWailaDataProvider
{
	private static String showStone = "udary.terrafirmacraft.grass.stone";
	...
	{
		if (accessor.getBlock() instanceof BlockGrass)
		{
			BlockGrass block = (BlockGrass)accessor.getBlock();

			if (config.getConfig(showStone))
			{
				int damage = itemStack != null ? itemStack.getItemDamage() : 0;
	
				if (block == TFCBlocks.grass2 || block == TFCBlocks.dryGrass2)
					damage += 16;
	...
	{
		TFCGrass dataProvider = new TFCGrass();
		
		reg.registerBodyProvider(dataProvider, BlockGrass.class);
